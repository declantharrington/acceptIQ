...
export function buildPIT({ report = {}, programContext = '', overrides = {} } = {}) {
  ...
  const findings = identifyFindings({ facts, merchantProfile, paymentsStack, metrics, commercialIntelligence, operationalIntelligence, industryIntelligence, riskIntelligence });
  const opportunities = identifyOpportunities({ facts, metrics, paymentsStack, commercialIntelligence, industryIntelligence, riskIntelligence });
  const commercialReasoning = buildCommercialReasoning({ merchantProfile, paymentsStack, metrics, commercialIntelligence, opportunities, risks: riskIntelligence.risks });
  const modulePlan = selectModules({ facts, metrics, findings, opportunities, riskIntelligence, dataQuality });
  const caseSummary = buildCaseSummary({ merchantProfile, paymentsStack, metrics, findings, opportunities, risks: riskIntelligence.risks, dataQuality, commercialReasoning });
  return { version:'PIT v2', generatedAt:new Date().toISOString(), facts, merchantProfile, paymentsStack, metrics, commercialIntelligence, operationalIntelligence, industryIntelligence, riskIntelligence, findings, opportunities, dataQuality, commercialReasoning, modulePlan, caseSummary };
}
