...
- pricingModel: one of "Single-rate", "Blended", "Interchange-plus", "Interchange-plus-plus", "Subscription", "Unknown" — only what the statement evidences. Distinguish Interchange-plus from Interchange-plus-plus using the fee breakdown structure, NOT the label alone (statements rarely state "IC+" or "IC++" outright):
  - Interchange-plus-plus (IC++): the statement itemises THREE separate cost layers — interchange, scheme/assessment fees (e.g. "Scheme fee", "Assessment fee", "Visa/Mastercard fee"), AND the provider's own margin — as distinct, separately-stated line items. The presence of a scheme/assessment fee as its OWN line item (not folded into a single "merchant service fee") is the key signal for IC++.
  - Interchange-plus (IC+): the statement itemises interchange separately, but bundles scheme/assessment fees together with the provider's margin into a single combined line (often labelled "merchant service fee" or "provider margin") — i.e. only TWO layers are separately visible, not three.
  - If a feeBreakdown line item explicitly named "Scheme fee(s)" or "Assessment fee(s)" appears separately from both interchange and a margin/service-fee line, classify as Interchange-plus-plus, not Interchange-plus.- providerRate (string): the rate the merchant's PROVIDER charges them — their own margin/markup — stated as printed, separate from interchange and scheme fees. For Interchange-plus / Interchange-plus-plus statements this is the acquirer margin or "merchant service fee" rate (e.g. "0.12%"), and it is typically the SAME rate for debit and credit. For Blended or Single-rate this is the blended rate(s) (e.g. "1.50% credit, 0.55% debit" or "1.50% all cards"). Use null if not determinable. This is DISTINCT from effectiveRate (the all-in cost of acceptance).
