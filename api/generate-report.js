...
//
// Methodology — uses the Knowledge Base §3 approved AVERAGE-to-cap delta:
...
// own credit card turnover (volume × credit% of card mix). This is the
...
const reformSavings = (() => {
  ...
  const CURRENT_AVG_RATE = 0.47; // % — Knowledge Base §3
  const NEW_CAP_RATE     = 0.30; // % — Knowledge Base §3, confirmed reform, 1 Oct 2026
...
- If chargeback data IS present in the STATEMENT FACTS below: analyse it properly — state the ratio/count plainly, compare to the commonly-cited scheme monitoring range (hedged, not asserted as exact), and explain the dollar and account-risk relevance. Two short subheaded paragraphs, as structured above.
